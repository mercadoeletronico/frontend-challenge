<template>
  <div class="bg-white border border-none rounded-lg overflow-hidden">
    <button @click="toggleAccordion"
      class="w-full px-6 py-3 text-left flex items-center justify-between bg-white transition-colors border-gray-200 focus:outline-none">
    ...
    <div v-show="isOpen" class="p-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="(address, index) in addresses" :key="index" class="rounded-lg p-4 shadow-md">
          <div class="mb-4">
          ...
          <div class="space-y-3">
            <div class="flex flex-col xl:flex-row xl:gap-8">
              <div class="flex items-start text-gray-600 text-sm mb-3 xl:mb-0 xl:flex-1">
                <font-awesome-icon icon="fa-solid fa-map-marker-alt"
                  class="mr-3 mt-0.5 text-gray-400 w-4 flex-shrink-0" />
                <span class="break-words">{{ address.address }}</span>
              </div>

              <div class="flex items-center text-gray-600 text-sm xl:flex-1">
                <font-awesome-icon icon="fa-regular fa-envelope" class="mr-3 text-gray-400 w-4 flex-shrink-0" />
                <span class="break-all">{{ address.contact.email }}</span>
              </div>
            </div>

            <div class="flex flex-col xl:flex-row xl:gap-8">
              <div class="flex items-center text-gray-600 text-sm mb-3 xl:mb-0 xl:flex-1">
                <font-awesome-icon icon="fa-regular fa-user" class="mr-3 text-gray-400 w-4 flex-shrink-0" />
                <span class="break-words">{{ address.contact.name }}</span>
              </div>

              <div class="flex items-center text-gray-600 text-sm xl:flex-1">
                <font-awesome-icon icon="fa-solid fa-phone" class="mr-3 text-gray-400 w-4 flex-shrink-0" />
                <span>{{ address.contact.phone }}</span>
              </div>
            </div>

            <div class="flex items-center text-gray-600 text-sm">
              <font-awesome-icon icon="fa-solid fa-fax" class="mr-3 text-gray-400 w-4 flex-shrink-0" />
